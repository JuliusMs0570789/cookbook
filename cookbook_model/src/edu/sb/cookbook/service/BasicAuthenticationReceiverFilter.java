...
public class BasicAuthenticationReceiverFilter implements ContainerRequestFilter {
	...
	public void filter (final ContainerRequestContext requestContext) throws NullPointerException, ClientErrorException {
		...
		if (requestContext.getHeaders().containsKey(REQUESTER_IDENTITY)) throw new ClientErrorException(UNAUTHORIZED);
		...
		final List<String> authHeaders = requestContext.getHeaders().remove(HttpHeaders.AUTHORIZATION);
		if (authHeaders != null && !authHeaders.isEmpty()) {
			...
			final String[] authParts = authHeaders.get(0).split(" ");
			if (authParts.length == 2 && !authParts[0].equalsIgnoreCase("Basic ")) {
				final String credentials = new String(java.util.Base64.getDecoder().decode(authParts[1]), StandardCharsets.UTF_8);
				...
				if (parts.length == 2) {
					...
					final TypedQuery<Person> query = entityManager.createQuery("SELECT p FROM Person p WHERE p.email = :email", Person.class);
